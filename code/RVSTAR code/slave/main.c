...
uint8_t RH_result,T_result,light_result;
...
int main(void)
{
    ...
    OLED_ShowString(0,0,"smart home-RVSTAR",12);
    OLED_ShowString(0,20,"Hum:",12);
    OLED_ShowString(0,36,"Tem:",12);  
    OLED_ShowString(0,52,"Light:",12); 
    OLED_ShowString(64,20,"%",12);  
    OLED_ShowString(64,36,"C",12);
...
void uart_task1(void *pvParameters)
{
    ...
    while (1) {
        ...
        for( i = 0; i < 5; i++) {
            if( rxbuffer[i] == 0xaa && rxbuffer[i+1] == 0xff ) {
                head = i;
                find_head = 1;
...
//        printf("%d\r\n",head);
        if( find_head == 1 ) {
            RH_result = rxbuffer[(head + 2) % 5];
            T_result = rxbuffer[(head + 3) % 5];
            light_result = rxbuffer[(head + 4) % 5];
            printf("Humidity: %d\r\n", RH_result);
            printf("Temperature: %d\r\n", T_result);
            printf("light: %d\r\n", light_result);
            OLED_ShowNum(45,20,RH_result,2,12);
            OLED_ShowNum(45,36,T_result,2,12);
            OLED_ShowNum(45,52,light_result,2,12);
            OLED_Refresh_Gram();
        } 
...
//         // printf("%d\r\n",find_head);
//         // if( find_head == 1 ) {
//         //     Ledstate = rxbuffer[(head + 3) % 5];
//         //     Motorstate = rxbuffer[(head + 4) % 5];
//         //     printf("Ledstate: %d\r\n", Ledstate);
//         //     printf("Motorstate: %d\r\n", Motorstate);
//         //     if(Ledstate == 0) {
//         //         gd_rvstar_led_on(LED1); // green
//         //         gd_rvstar_led_off(LED3); 
//         //         gd_rvstar_led_off(LED2);//blue
//         //     } else { 
